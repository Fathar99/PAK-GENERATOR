...
- **Barcode keaslian dokumen** — setiap halaman kop surat memuat barcode
  ...
  dokumen tidak diubah/dipalsukan. Barcode dibuat langsung di browser
  (JsBarcode) dan "ditukar" ke dalam file `.docx` lewat manipulasi ZIP —
  pendekatan ini dipilih karena library alternatif (docxtemplater image
  module) punya bug lama yang belum diperbaiki di browser modern.
...
│   └── barcode-placeholder.png # Placeholder, ditukar otomatis saat dokumen dibuat
├── js/
...
> (jsDelivr) saat halaman dibuka, jadi pengguna perlu koneksi internet aktif.
