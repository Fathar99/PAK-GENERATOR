...
(function (root, factory) {
   ...
   */
  function hitungPenetapan({ jenjangKey, akDasarDiberikan, akJFLama, akKonversiBaru, namaPangkatBerikutnya, namaJenjangBerikutnya }) {
    const jenjang = KOEFISIEN_JENJANG[jenjangKey];
    ...
    const kataDasarPangkat = cukupPangkat ? "DAPAT" : "BELUM DAPAT";
    const tujuanPangkat = namaPangkatBerikutnya ? ` MENJADI ${namaPangkatBerikutnya}` : "";
    const kesimpulanPangkat = `${kataDasarPangkat} DIPERTIMBANGKAN UNTUK KENAIKAN PANGKAT SETINGKAT LEBIH TINGGI${tujuanPangkat}.`;

    let kesimpulanJenjang = "";
    if (minJenjang !== null && minJenjang !== undefined) {
      const kataDasarJenjang = cukupJenjang ? "DAPAT" : "BELUM DAPAT";
      const tujuanJenjang = namaJenjangBerikutnya ? ` MENJADI ${namaJenjangBerikutnya}` : "";
      kesimpulanJenjang = `${kataDasarJenjang} DIPERTIMBANGKAN UNTUK KENAIKAN JENJANG JABATAN SETINGKAT LEBIH TINGGI${tujuanJenjang}.`;
    }
    ...
    return {
      ...
      kesimpulanPangkat,
      kesimpulanJenjang,
      // dipertahankan untuk kompatibilitas mundur (dipakai skrip lama)
      kesimpulan: kesimpulanJenjang ? `${kesimpulanPangkat} ${kesimpulanJenjang}` : kesimpulanPangkat,
    };
  }
